...
if (isset($_FILES['foto']) && !empty($_FILES['foto']['name'])) {
    ...
    if (in_array($ekstensi, $allowed_extensions) === true) {
        ...
        if ($ukuran < 1500000) {
            ...
            mysqli_query($koneksi, "INSERT INTO rs VALUES ('', '$nama', '$alamat', '$deskripsi', '$latitude', '$longitude', '$image')");
            if ($koneksi) {
                echo "<script>alert('Data Berhasil Ditambahkan!'); window.location = 'tampil_data_rs.php'</script>";
            } else {
                echo "<script>alert('Data Gagal Ditambahkan!'); window.location = 'tampil_data_rs.php'</script>";
            }
        } else {
            // kalau file melebihi batas
            echo "<script>alert('Ukuran file terlalu besar!'); window.location = 'tampil_data_rs.php'</script>";
        }
    } else {
        // kalau file tidak png atau jpg
        echo "<script>alert('Format file tidak sesuai!'); window.location = 'tampil_data_rs.php'</script>";
    }
} else {
    // If $_FILES['foto'] is not set or empty
    echo "<script>alert('File foto tidak ada atau kosong!'); window.location = 'tampil_data_rs.php'</script>";
}
?>
