...
if (isset($_FILES['foto']) && !empty($_FILES['foto']['name'])) {
    ...
    if (in_array($ekstensi, $allowed_extensions) === true) {
        ...
        if ($ukuran < 1500000) {
            ...
            mysqli_query($koneksi, "insert into puskesmas values('','$nama','$alamat','$deskripsi','$latitude','$longitude','$image')");
            if ($koneksi) {
                echo "<script>alert('Data Berhasil Ditambahkan!'); window.location = 'tampil_data_puskesmas.php'</script>";
            } else {
                echo "<script>alert('Data Gagal Ditambahkan!'); window.location = 'tampil_data_puskesmas.php'</script>";
            }
        } else {
    ...
    echo "<script>alert('File foto tidak ada atau kosong!'); window.location = 'tampil_data_puskesmas.php'</script>";
}
